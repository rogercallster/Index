...
public class Manager {
	...
	public static void manager (String string)throws Exception{
		...
		String Path="/pub/FreeBSD/CERT/packages";
		   ...
		   public static void itterable(FTPSession s,String Path)
		   {
			   String StrOfCurrDir=Ad.marshal((s.list(Path).sync())).toString();
			   ...
			   {	   
				 ...
				 if(O.name!=null)
				   ...
				   AttributeObjects.SIZE.putKey(String.valueOf(O.size),addr);
				   		if(O.dir)
				   		{
					   //System.out.println("Hello World    "+O.dir);
					   //System.out.println( O.name + "path = " + Path + " O.dir = "+O.dir);   
				    itterable( s, Path+"/"+O.name);
			        
				       }
				    }
			    }//end of for loop ///
//			   inside add unique/pub/FreeBSD/ports/i386/packages-9.2-release/.staging_reldirSize of table is 1092164
